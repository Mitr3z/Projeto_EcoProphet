...
def exportar_csv():
    ...
    if tipo_relatorio == 'analise':
        ...
        if previsao:
            ...
            # Mesclar com o DataFrame original para obter todas as colunas
            df_tipo = df_tipo.merge(df_residuos, on=['Ano da geração', 'Tipo de Resíduo'], how='left')
        else:
...
def exportar_pdf():
    ...
    for _, row in df_tipo.iterrows():
        ...
        campos = [
            f"CNPJ: {row['CNPJ do gerador']}",
            f"Razão Social: {row['Razão Social do gerador']}",
            f"Estado: {row['Estado']}",
            f"Município: {row['Município']}",
            f"Categoria: {row['Categoria de Atividade']} (Código: {row['Código da Categoria']})",
            f"Detalhe: {row['Detalhe']} (Código: {row['Código do Detalhe']})",
            f"Cód. Resíduo: {row['Cód. Resíduo']}",
            f"Quantidade: {row['Quantidade Gerada']:.2f} {row['Unidade']}".replace('.', ','),
            f"Classificação: {row['Classificação Resíduo']}",
            f"Situação Cadastral: {row['Situação Cadastral']}"
        ]
